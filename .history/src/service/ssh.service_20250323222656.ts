...
import { Client } from 'ssh2'; // Import only what's needed
...
class SSHConnection {
  ...
  async runPersistentCommand(command: string): Promise<string> {
    ...
    return new Promise(resolve => {
      ...
      this.shellStream.removeAllListeners('data'); // 清除旧的监听器
    ...
    return new Promise((resolve, reject) => {
      ...
      this.conn.exec(command, (err, stream) => {
        ...
        stream
          ...
          .on('error', err => reject({ success: false, error: err.message })) // 添加错误捕获
          .on('close', code => {
            code === 0
  ...
  close(): string {
    if (this.isConnected && this.shellStream) {
      try {
        ...
        return `关闭 SSH 连接时发生错误: ${err.message}`;
      }
      this.shellStream = null;
      this.isConnected = false;
      return 'SSH 连接已关闭';
